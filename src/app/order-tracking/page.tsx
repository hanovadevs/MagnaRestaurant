...
import { 
  CheckCircle, MapPin, Truck, Utensils, 
  Phone, ArrowLeft, Star, Coffee, MessageSquare, Loader2
} from "lucide-react";
import Button from "@/components/Button";
import Link from "next/link";

const steps = [
  { id: 1, label: "Order Received", Icon: CheckCircle },
  { id: 2, label: "Preparing Food", Icon: Utensils },
  { id: 3, label: "On the Way", Icon: Truck },
  { id: 4, label: "Delivered", Icon: Star },
];

const TrackingContent = () => {
  const searchParams = useSearchParams();
  const orderId = searchParams.get("id") || "MAG-ORDER";
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-6 rounded-[32px] shadow-sm border border-black/5 flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <div className="w-16 h-16 bg-stone-100 rounded-2xl overflow-hidden shadow-inner flex items-center justify-center">
                <img src="https://images.unsplash.com/photo-1633332755192-727a05c4013d?q=80&w=200&auto=format&fit=crop" alt="Courier" className="w-full h-full object-cover" />
              </div>
              <div>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-wood-dark p-8 rounded-[40px] shadow-2xl space-y-4 text-white">
            <h4 className="font-elegant font-bold text-lg">Help & Support</h4>
...
const TrackingPage = () => {
  return (
    <div className="min-h-screen bg-stone-50 py-12 px-4">
      <Suspense fallback={<div className="flex items-center justify-center p-20"><Loader2 className="animate-spin w-10 h-10 text-primary" /></div>}>
        <TrackingContent />
      </Suspense>
    </div>
  );
